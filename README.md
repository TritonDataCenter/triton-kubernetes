...
$ ./k8s-triton-supervisor.sh
	...
	Usage:
		./k8s-triton-supervisor.sh [-c|-e]
	Options:
		-c  create a cluster manager on Triton
		-e  create and add a Kubernetes environment to an existing cluster manager
```

As you can see there are two options here, `-c` to create cluster manager and `-e` to create/add an environment to the cluster manager.

> HELP: There is also a debugging flag `--cleanAll true` which will delete the entire setup and reset this package. Hosts still have to be removed manually.
