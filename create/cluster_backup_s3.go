...
const (
	clusterBackupS3TerraformModulePath = "terraform/modules/k8s-backup-s3"
)

type s3ClusterBackupTerraformConfig struct {
...
func newS3ClusterBackup(selectedClusterKey string, currentState state.State) (string, error) {
	nonInteractiveMode := viper.GetBool("non-interactive")
	baseConfig, err := getBaseClusterBackupTerraformConfig(clusterBackupS3TerraformModulePath, selectedClusterKey)
	if err != nil {
		return "", err
	}

	cfg := s3ClusterBackupTerraformConfig{
